...
def get_vertex_colors(model, mesh, frame_idx=0, view_dir=None):
    ...
    # appa_code = model.appearance_code(torch.Tensor([frame_idx]).long().to(model.device))
    # appa_code = appa_code.expand(xyz_query.shape[0],-1)
    ...
    # xyz_embedded = torch.cat([xyz_embedded, dir_embedded, env_code, appa_code],-1)
