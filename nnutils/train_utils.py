...
class v2s_trainer():
    ...
    def define_model(self, data_info):
        ...
        self.model = moda.moda(opts, data_info)
    ...
    def init_training(self):
        ...
        self.model.module.final_steps = self.num_epochs * \
                                min(200,len(self.trainloader)) * opts.accu_steps  #120 * * 1
        ...
        for name,p in self.model.named_parameters():
            ...
            elif 'nerf_dis' in name:
                params_nerf_dis.append(p)
            ...
            elif 'appearance_code' in name:
                params_appearance_code.append(p)
        ...
        self.scheduler = torch.optim.lr_scheduler.OneCycleLR(self.optimizer,\
                      ...
                      10*opts.learning_rate, # params_nerf_beta_feat
                         ...
                         opts.learning_rate, # params_nerf_dis
        ...
        lr_nerf_root_rts*opts.learning_rate, # params_root_code
                         ...
                         opts.learning_rate, # params_appearance_code
    ...
    def load_network(self,model_path=None, is_eval=True, rm_prefix=True):
        ...
        if rm_prefix and (opts.lbs or opts.neudbs) and states['bones'].shape[0] != self.model.bones.shape[0]:
            ...
            if opts.nerf_dis:
                states = self.rm_module_prefix(states, prefix='nerf_dis')  #not sure
        ...
        with torch.no_grad():
            ...
            for j in range(0, len(idx_render), chunk):
                ...
                for k, v in rendered.items():
                    rendered_seq[k] += [v]
                if 'dis_reg' in rendered_seq:
                    del rendered_seq['dis_reg']
                if 'dis_reg_forward' in rendered_seq:
                    del rendered_seq['dis_reg_forward']
            ...
            if (opts.lbs or opts.neudbs):
                ...
                bones_rst,_ = correct_bones(self.model, bones_rst, neudbs=opts.neudbs)
    ...
    def train(self):
        ...
        if (opts.lbs or opts.neudbs) and opts.nerf_skin:
            del self.model.module.nerf_models['nerf_skin']
        # if opts.lbs and opts.nerf_dis:
        #     del self.model.module.nerf_models['nerf_dis']
        ...
        if opts.freeze_coarse:  # have not added nerf_dis here 
            ...
            # self.model.module.dis_xyz_wt = \
            #     grab_xyz_weights(self.model.module.nerf_dis, clone=True)
        ...
        for epoch in range(0, self.num_epochs):
            ...
            print('start training epoch {:d}'.format(epoch))
            self.train_one_epoch(epoch, log)
            
            message = 'saving the model at the end of epoch {:d}, iters {:d}'.\
                              format(epoch, self.model.module.total_steps)
            print(message)
            log_name = os.path.join(opts.checkpoint_dir, opts.logname, 'loss_log.txt')
    ...
    def reset_hparams(self, epoch):
        ...
        if (opts.lbs or opts.neudbs) and opts.model_path=='' and \
                        (epoch==int(self.num_epochs*opts.reinit_bone_steps) or\
                         epoch==0 or\
                         epoch==int(self.num_epochs*opts.warmup_steps)//2):
            reinit_bones(self.model.module, mesh_rest, opts.num_bones, opts.neudbs)
        ...
        if (opts.lbs or opts.neudbs) and opts.model_path!='':
            self.model.module.nerf_models['bones'] = self.model.module.bones
            # self.model.module.nerf_models['nerf_dis'] = self.model.module.nerf_dis

        # add nerf-skin when the shape is good
        if (opts.lbs or opts.neudbs) and opts.nerf_skin and \
                epoch==int(self.num_epochs*opts.dskin_steps):
            self.model.module.nerf_models['nerf_skin'] = self.model.module.nerf_skin
        # if opts.lbs and opts.nerf_dis and \
        #         epoch==int(self.num_epochs*opts.dskin_steps):
        #     self.model.module.nerf_models['nerf_dis'] = self.model.module.nerf_dis  # not sure
    ...
    def broadcast(self):
        ...
        """
        dist.barrier()
        if (self.opts.lbs or self.opts.neudbs):
    ...
    def clip_grad(self, aux_out):
        ...
        if self.model.module.shape_update == 1:
            ...
            self.zero_grad_list(grad_nerf_dis) #
        ...
        aux_out['nerf_dis_g']      = clip_grad_norm_(grad_nerf_dis,      .1*clip_scale)
        ...
        aux_out['appearance_code_g']      = clip_grad_norm_(grad_appearance_code,      .1*clip_scale)
    ...
    def extract_mesh(model,chunk,grid_size,
        ...
        if mesh_dict_in is None:
            ...
            message = 'fraction occupied:', (vol_o > threshold).float().mean()
            print(message)
