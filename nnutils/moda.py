...
from nnutils.nerf import Embedding, NeRF, DQ_RTHead, RTHead, SE3head, RTExplicit, Encoder,\
...
flags.DEFINE_bool('appearance_code', False, 'whether to use appearance code for each frame')
flags.DEFINE_bool('env_fourier', True, 'whether to use fourier basis for env')
flags.DEFINE_bool('appearance_fourier', True, 'whether to use fourier basis for appearance')
...
flags.DEFINE_bool('neudbs', True, 'use neudbs for backward warping 3d flow')
...
flags.DEFINE_bool('nerf_dis', False, 'use displacement field')
...
flags.DEFINE_float('learning_rate', 5e-4, 'learning rate')  # 5e-4
...
flags.DEFINE_float('clip_scale', 10, 'grad clip scale') # 100
...
flags.DEFINE_bool('use_corr', False, 'whether to use correspondence loss')
flags.DEFINE_float('corr_wt', 0.01, 'by default, weight for corr loss')
...
flags.DEFINE_bool('use_ot', True, 'whether to use optimal transport in feat_match')
flags.DEFINE_bool('s3im_loss', False,'whether to use s3im loss')
flags.DEFINE_float('s3im_wt',  0.01, 'weight for s3im loss')
flags.DEFINE_bool('ppr_eikonal', False, 'whether to use ppr eikonal')
flags.DEFINE_bool('rgb_filter', False,'whether to use texture filtering')
flags.DEFINE_float('scale_rgb', 1.3, 'scaling parameter for texture filtering')
...
class moda(nn.Module):
    def __init__(self, opts, data_info):
        ...
        self.obj_scale = float(near_far_to_bound(self.near_far)) / 0.3 
        self.near_far = self.near_far / self.obj_scale
        self.near_far_base = self.near_far.clone()
        ...
        if opts.appearance_code: #add frame-specficit appearance code
            appearance_code_dim = 128
            if opts.appearance_fourier: 
                self.appearance_code = FrameCode(self.num_freqs, appearance_code_dim, self.data_offset, scale=1)
            else:
                self.appearance_code = embed_net(self.num_fr, appearance_code_dim) #num_fr is all of the frames in multi-vids
        else:
            appearance_code_dim = 0
        self.nerf_coarse = NeRF(in_channels_xyz=in_channels_xyz, 
                                in_channels_dir=in_channels_dir+env_code_dim+appearance_code_dim,
                                init_beta=opts.init_beta, enable_semantic=False)
        ...
        # set motion model
        t_embed_dim = opts.t_embed_dim  # 128 dimension of the pose code
        if opts.frame_code:
...
#                                    D=5,W=128,
            ...
            if opts.nerf_dis:
                self.nerf_dis = NeRF(in_channels_xyz=in_channels_xyz+t_embed_dim,
#                                    D=5,W=128,
                                    D=5,W=128,
                     in_channels_dir=0, out_channels=3, 
                     raw_feat=True, in_channels_code=t_embed_dim)
                self.nerf_models['nerf_dis'] = self.nerf_dis
    ...
    def forward_default(self, batch):
        ...
        ### for s3im loss
        if opts.s3im_loss:
            s3im_loss = opts.s3im_wt*rendered['s3im_loss']
            total_loss = total_loss + s3im_loss
            aux_out['s3im_loss'] = s3im_loss
        ...
        # viser loss
        if opts.use_embed:
            feat_err_samp = rendered['feat_err']* opts.feat_wt   #0
            if opts.loss_flt:
            ...
            if opts.use_corr:
                corr_err_samp= rendered['corr_err'] * opts.corr_wt
                if opts.loss_flt:
                    corr_err_samp[invalid_idx] *= 0
                corr_loss = corr_err_samp
                if opts.rm_novp:
                    corr_loss = corr_loss * rendered['sil_coarse'].detach()
                corr_loss = corr_loss[sil_at_samp>0].mean()
                total_loss = total_loss + corr_loss
                aux_out['corr_loss'] = corr_loss
        ...
        # regularization of nerf_dis
        if 'dis_reg' in rendered.keys():
            dis_bw_reg = rendered['dis_reg'].mean()
            total_loss = total_loss + dis_bw_reg 
        if 'dis_reg_forward' in rendered.keys():
            dis_fw_reg = rendered['dis_reg_forward'].mean()
            total_loss = total_loss + dis_fw_reg 
        ...
        if opts.eikonal_wt > 0:
            ekl_loss = opts.eikonal_wt * eikonal_loss(self.nerf_coarse, self.embedding_xyz, 
                        rendered['xyz_canonical_vis'], self.latest_vars['obj_bound'], opts.ppr_eikonal)
        ...
        if (opts.lbs or opts.neudbs) and opts.bone_loc_reg>0:
            print("here")
            bones_rst = self.bones
            bones_rst,_ = correct_bones(self, bones_rst, neudbs=opts.neudbs)
            ...
            if len(mesh_rest.vertices)>100: # not a degenerate mesh
                mesh_rest = pytorch3d.structures.meshes.Meshes(
                        verts=torch.Tensor(mesh_rest.vertices[None]).to(self.device),
                        faces=torch.Tensor(mesh_rest.faces[None]).to(self.device))
        ...
        if opts.freeze_coarse:
            ...
            skin_xyz_wt_loss = 100*compute_xyz_wt_loss(self.skin_xyz_wt, 
                                                        skin_xyz_wt_curr)
            if opts.nerf_dis:
                dis_xyz_wt_curr = grab_xyz_weights(self.nerf_dis)
                dis_xyz_wt_loss = 100*compute_xyz_wt_loss(self.dis_xyz_wt, 
                                                            dis_xyz_wt_curr)
                aux_out['dis_xyz_wt_loss'] = dis_xyz_wt_loss
            ...
            total_loss = total_loss + shape_xyz_wt_loss + skin_xyz_wt_loss + feat_xyz_wt_loss
            if opts.nerf_dis:
                total_loss = total_loss + dis_xyz_wt_loss

        # save some variables
        if opts.lbs or opts.neudbs:
    ...
    def nerf_render(self, rtk, kaug, embedid, nsample=256, ndepth=128):
        ...
        for i in range(0, bs_rays, opts.chunk):
            ...
            if self.progress > opts.fine_steps:  #1.1
                #print('into Fine iter')
                self.use_fine = True
            else:
                self.use_fine = False
    ...
    def sample_pxs(self, bs, nsample, Rmat, Tmat, Kinv,
        ...
        rand_inds, xys = sample_xy(self.img_size, bs, nsample+nsample_a, self.device,
                               return_all= not(self.training), lineid=lineid) #(512, 512, 6+24, False, lineid.shape=512)==(512,30),(512,30,2)
        ...
        if is_active:
            ...
            if opts.lineload:
                ...
                frameid   =   frameid.view(2,-1).to(self.device)
                frameid_a = frameid_a.view(2,-1).to(self.device)
                frameid_sub   =   frameid_sub.view(2,-1).to(self.device)
                frameid_sub_a = frameid_sub_a.view(2,-1).to(self.device)
                dataid   =   dataid.view(2,-1).to(self.device)
                dataid_a = dataid_a.view(2,-1).to(self.device)
                errid   =     errid.view(2,-1).to(self.device)
                errid_a   = errid_a.view(2,-1).to(self.device)
    ...
    def update_delta_rts(self, rays):
        ...
        """
        opts = self.opts
        bones_rst, bone_rts_rst = correct_bones(self, self.nerf_models['bones'], neudbs=opts.neudbs)
        self.nerf_models['bones_rst']=bones_rst

        # delta rts
        rays['bone_rts'] = correct_rest_pose(opts, rays['bone_rts'], bone_rts_rst, opts.neudbs)

        if 'bone_rts_target' in rays.keys():       
            rays['bone_rts_target'] = correct_rest_pose(opts, 
                                            rays['bone_rts_target'], bone_rts_rst, opts.neudbs)

        if 'bone_rts_dentrg' in rays.keys():
            rays['bone_rts_dentrg'] = correct_rest_pose(opts, 
                                            rays['bone_rts_dentrg'], bone_rts_rst, opts.neudbs)
    ...
    def update_rays(self, rays, is_pair, dataid, frameid_sub, embedid, xys, Kinv):
        ...
        if is_pair:
            ...
            if opts.flowbw:
                ...
                rays['time_embedded_target'] = time_embedded_target.repeat(1,
                                                            rays['nsample'],1)
            elif (opts.lbs or opts.neudbs) and self.num_bone_used>0:
        ...
        if (opts.lbs or opts.neudbs) and self.num_bone_used>0:
        ...
        if opts.env_code:
            ...
            rays['env_code'] = rays['env_code'].repeat(1,rays['nsample'],1)  #(bs, nsample, env_dim=64)
        
        if opts.appearance_code:
            rays['appearance_code'] = self.appearance_code(embedid)[:,None]
            rays['appearance_code'] = rays['appearance_code'].repeat(1,rays['nsample'],1)
