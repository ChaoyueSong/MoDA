...
class NeRF(nn.Module):
    def __init__(self,
                 ...
                 activation=nn.ReLU(True), in_channels_code=0, enable_semantic=False):
        """
        adapted from https://github.com/kwea123/nerf_pl/blob/master/models/nerf.py
        ...
        if enable_semantic:
            self.semantic_linear = nn.Sequential(fc_block(W, W // 2), nn.Linear(W // 2, 1))
...
#                    nn.init.xavier_uniform_(m.weight)
    ...
    def forward(self, x ,xyz=None, sigma_only=False):
        ...
        if self.raw_feat:
            ...
            rgb = rgb.sigmoid()
            # if use_semantic:
            #     out = torch.cat([rgb, sigma, sem_logits], -1)
            # else:
...
class DQ_RTHead(NeRF):
    ...
    def forward(self, x):
        ...
        x = super(DQ_RTHead, self).forward(x)
        bs = x.shape[0]
        ...
        if self.use_quat:
            rquat=rts[:,3:7] # pre: 7
            # rquat=axis_angle_to_quaternion(rquat) 
            rquat=F.normalize(rquat,2,-1) # 10-1 
            # print(rquat)
            T_real_parts = tmat.new_zeros(tmat.shape[:-1] + (1,))
            tquat = torch.cat((T_real_parts, tmat), -1)

            # R_quat and T_quat to dq
            dq_r = rquat
            dq_d = 1/2 * transforms.quaternion_raw_multiply(tquat, rquat)
            dq = torch.cat((dq_r, dq_d),-1) 
...
#        self.num_freq = num_freq
    ...
    def forward(self, fid):
        ...
        """
        bs = fid.shape[0]
        vid, tid = fid_reindex(fid, self.num_vids, self.vid_offset)  #re-index absolute frameid {0,....N} to subsets of video id and relative frameid
